...
### "This operation requires the FTS Admin permission set" Error Persists

**Symptom:** When deploying a configuration in the Configuration Wizard (selecting an object and clicking **Deploy Settings**), the error **"This operation requires the FTS Admin permission set"** appears — even though the **Flexible Team Share - Admin** permission set has already been assigned to the user.

**Cause:** This is a Salesforce permission caching quirk. After a permission set is assigned, Salesforce sometimes continues to evaluate against the user's previous permission state for the active session, so the newly granted access is not recognized immediately.

**Solution:**
<Steps>
1. Verify the **Flexible Team Share - Admin** permission set (or the Admin Permission Set Group) is assigned to the user under **Setup** > **Permission Set Groups** / **Permission Sets**
2. Perform a hard refresh in the browser to force Salesforce to re-evaluate permissions:
...
3. If the error persists, log out and log back in to start a fresh session
4. Retry the **Deploy Settings** action — the operation should now succeed
</Steps>
